      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="text-2xl font-bold text-[#1E2A4A]">{card.value}</p>
            ...
            {topItems.map((item, i) => (
              ...
              >
                <span className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full bg-[#1E2A4A] text-white text-xs flex items-center justify-center flex-shrink-0">
                    {i + 1}
                  </span>
                  <span className="text-gray-700">{item.name}</span>
                </span>
                <span className="font-medium text-[#F26522]">{item.count} مرة</span>
              </li>
            ))}
          </ol>
        </div>
